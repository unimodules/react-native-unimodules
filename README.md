...
- If you need to exclude some of the unimodules that you are not using but they got installed by your other dependencies (like `expo`), then you can pass in `exclude` param for this. For example, if you want to exclude `expo-face-detector`, you may want to use this: `includeUnimodulesProjects([exclude: ['expo-face-detector']])` and `addUnimodulesDependencies([exclude: ['expo-face-detector']])`
